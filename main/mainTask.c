...
void Enter(ELABEL_STATE state)
{
    ...
    case Focus:
        needFlashEpaper = true;
        _button_tick_buf = TimeTick;
        focusTaskTimeCountdown = TimeCountdown;
    ...
    while(1)
    {
        ...
        case Preparing:
            ...
            if(tasklen && !last_tasklen)
            {
                ...
                _ui_flag_modify(ui_Label8, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);

                _ui_flag_modify(ui_Label6, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_ADD);
                _ui_flag_modify(ui_Label12, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_ADD);
            }
            else if(!tasklen && last_tasklen)
            {
                _ui_flag_modify(ui_Roller1, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_ADD);
                _ui_flag_modify(ui_Label8, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_ADD);

                _ui_flag_modify(ui_Label6, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);
                _ui_flag_modify(ui_Label12, LV_OBJ_FLAG_HIDDEN, _UI_MODIFY_FLAG_REMOVE);
            }
...
{
    if(send_param->state == 0 || true) return;
...
bool APP_FocusState_update()
{
    ...
    bool isFocus = get_global_data()->m_focus_state->is_focus;
    int FocusId = get_global_data()->m_focus_state->focus_task_id; 

    if(label_state == Focus && !isFocus)
    {
        // delete_task(&task_list,chosenTaskNum);//删除当前任务
        // tasks2str(task_list);
        // lv_roller_set_options(ui_Roller1, taskstr, LV_ROLLER_MODE_NORMAL);
    ...
    else if(label_state != Focus && isFocus)
    {
        TodoList* todolist = get_global_data()->m_todo_list;
